...
/** Test zstd (pure-Java aircompressor, works on the JVM); this module's tests share this one instance. */
internal val TEST_ZSTD = Zstd { input, expected ->
...
/**
 * Verifies the reader against a fixture generated by real restic (src/test/resources/repo,
 * password test123, v2/zstd).
 * Backup tree: tmp/twig_rsrc/{big.txt(4800B), hello.txt, sub/note.md}
 */
class ResticRepoTest {
    ...
    fun readsCompressedMultiBlobFile() {
        // big.txt is 4800B and gets compressed under v2; verify sha256 matches the reference
        val repo = open("test123")
        val rfs = ResticFileSystem(repo, "restic")
        val snap = rfs.list(rfs.root()).first().path
